...
def load_data(filepath, input_columns, output_columns, output_dir='./processed_data'):
    """
    Load data from Excel or CSV file (with automatic Excel conversion).

    This function automatically detects if the input is Excel or CSV:
    - If Excel: Automatically detects process type and converts to CSV first
    - If CSV: Loads directly

    Args:
        filepath (str): Path to Excel (.xlsx, .xls) or CSV file
        input_columns (list): Names of input columns to use as features
        output_columns (list): Names of output columns to use as targets
        output_dir (str): Directory where converted CSV will be saved (default: './processed_data')

    Returns:
        tuple: (X, y) as numpy arrays

    Examples:
        >>> # Load from CSV directly
        >>> X, y = load_data('data.csv', ['col1', 'col2'], ['target'])

        >>> # Load from Excel (automatic conversion and detection)
        >>> X, y = load_data('laser.xlsx', ['las_wa', 'las_panel'], ['las_papos'])
    """
    # Check if file is Excel
    if filepath.endswith(('.xlsx', '.xls')):
        print(f"Excel file detected: {filepath}")
        print("Auto-detecting process type and converting to CSV...")

        # Import converter only when needed
        from .excel_to_csv_converter import convert_excel_auto

        # Convert Excel to CSV with auto-detection
        csv_filepath = convert_excel_auto(filepath, output_dir)
        print(f"Conversion complete. Loading data from: {csv_filepath}")

        # Load from the converted CSV
        filepath = csv_filepath

    # Load CSV data (either original or converted)
    return load_csv_data(filepath, input_columns, output_columns)
