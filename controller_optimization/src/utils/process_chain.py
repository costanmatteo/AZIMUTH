...
class ProcessChain(nn.Module):
    ...
    def __init__(self, processes_config, target_trajectory, policy_config=None, device='cpu'):
        ...
        # Dense Connections: pass actions from ALL previous policies to each policy
        # Policy₁.input = [o₁, scenario_emb]           # unchanged
        # Policy₂.input = [o₂, scenario_emb, a₂]       # + action from Policy₁
        # Policy₃.input = [o₃, scenario_emb, a₂, a₃]   # + actions from Policy₁ and Policy₂
        # This creates shorter gradient paths: ∂L/∂a₄ · ∂a₄/∂a₂ · ∂a₂/∂θ₁
        self.use_dense_connections = policy_config.get('use_dense_connections', True)
        ...
        # Print dense connections status
        if self.use_dense_connections:
            print(f"  Dense connections: ENABLED (actions from all previous policies)")
        else:
            print(f"  Dense connections: disabled (standard chain)")
        ...
        for i in range(len(processes_config) - 1):
            # Input to policy: [prev_outputs_mean, prev_outputs_var, scenario_embedding, prev_actions...]
            prev_output_dim = processes_config[i]['output_dim']
            policy_input_size = prev_output_dim + prev_output_dim  # mean + var

            # Add scenario embedding dimension if encoder is enabled
            if self.use_scenario_encoder:
                policy_input_size += self.scenario_embedding_dim

            # Dense Connections: add dimensions for all previous policy outputs
            # Policy 0: no previous actions
            # Policy 1: + a₂ (output of Policy 0 = n_controllable of process 1)
            # Policy 2: + a₂ + a₃ (outputs of Policy 0 and 1)
            dense_connection_dims = 0
            if self.use_dense_connections and i > 0:
                # Sum of controllable outputs from all previous policies
                for j in range(i):
                    # Policy j outputs controllable inputs for process j+1
                    dense_connection_dims += self.controllable_info_per_process[j + 1]['n_controllable']
                policy_input_size += dense_connection_dims
            ...
            if next_preprocessor.input_min is not None and next_preprocessor.input_max is not None:
                ...
                print(f"    Input dim: {policy_input_size}" +
                      (f" (includes {dense_connection_dims} from dense connections)" if dense_connection_dims > 0 else ""))
    ...
    def forward(self, batch_size=1, scenario_idx=0):
        ...
        # Dense Connections: track all actions generated by previous policies
        # previous_actions[j] = output of policy j (controllable inputs for process j+1)
        previous_actions = []
        ...
        if ProcessChain.debug:
            ...
            print(f"Dense connections: {'ENABLED' if self.use_dense_connections else 'disabled'}")
        ...
        for i, process_name in enumerate(self.process_names):
            ...
            if i > 0:
                ...
                # Concatenate: [prev_outputs_mean, prev_outputs_var, scenario_embedding, prev_actions...]
                policy_input_parts = [
                    prev_outputs_mean,
                    prev_outputs_var
                ]
                ...
                # Dense Connections: add all previous policy actions
                # Policy i-1 receives actions from policies 0, 1, ..., i-2
                if self.use_dense_connections and len(previous_actions) > 0:
                    policy_input_parts.extend(previous_actions)
                    if ProcessChain.debug:
                        print(f"    Dense connections: adding {len(previous_actions)} previous action tensors")
                        for idx, act in enumerate(previous_actions):
                            print(f"      Action {idx}: shape={act.shape}, mean={act.mean().item():.6f}")
                ...
                # Dense Connections: store this policy's output for subsequent policies
                previous_actions.append(controllable_outputs)
