...
if __name__ == '__main__':
    ...
    # Random embeddings
    embeddings = np.random.randn(n_scenarios, embedding_dim)

    # Structural params with some structure
    structural_params = np.zeros((n_scenarios, n_params))
    structural_params[:, 0] = np.linspace(20, 30, n_scenarios) + np.random.randn(n_scenarios) * 0.5  # Temperature 1
    structural_params[:, 1] = np.linspace(25, 35, n_scenarios) + np.random.randn(n_scenarios) * 0.5  # Temperature 2

    scenario_indices = np.arange(n_scenarios)
    param_names = ['AmbientTemp', 'MicroetchTemp']

    # Create test directory
    test_dir = Path('test_embedding_plots')
    test_dir.mkdir(exist_ok=True)

    # Generate plots
    plot_paths = generate_all_embedding_plots(
    ...
    print(f"\n✓ Test completed! Plots saved in {test_dir}")
