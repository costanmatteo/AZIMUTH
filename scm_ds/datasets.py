...
ds_scm_laser = SCMDataset(
    ...
    singles={
        ...
        "SIGMA_M0": lambda rng, n: np.full(n, 0.05),
        "C_T":      lambda rng, n: np.full(n, 0.003),
        "C_I":      lambda rng, n: np.full(n, 0.06),
        "KAPPA":    lambda rng, n: np.full(n, 0.003),
        "SIGMA_A":  lambda rng, n: np.full(n, 0.025),
        "SIGMA_D":  lambda rng, n: np.full(n, 0.012),
...
ds_scm_plasma = SCMDataset(
    ...
    singles={
        ...
        "C_P":      lambda rng, n: np.full(n, 0.0015),
        "SIGMA_A":  lambda rng, n: np.full(n, 0.006),
...
ds_scm_galvanic = SCMDataset(
    ...
    singles={
        ...
        "SIGMA_G": lambda rng, n: np.full(n, 0.06),
...
ds_scm_microetch = SCMDataset(
    ...
    singles={
        ...
        # Noise parameters
        "SIGMA_M": lambda rng, n: np.full(n, 0.18),     # 18% multiplicative noise (3x)
        "NU":      lambda rng, n: np.full(n, 5.0),      # Student-t degrees of freedom
        "S_T":     lambda rng, n: np.full(n, 0.9),      # 0.9 μm additive noise (3x)
